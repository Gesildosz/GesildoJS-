...
export default function AnomalyReportByVehiclePage() {
  const searchParams = useSearchParams()
  const vehicleIdFilter = searchParams.get("vehicleId") // Obtém o vehicleId da URL
  ...
  useEffect(() => {
    if (vehicleIdFilter) {
      const filtered = anomalies.filter((a) => a.vehicleId === vehicleIdFilter)
      setFilteredAnomalies(filtered)
      if (filtered.length === 0) {
        toast({
          title: "Nenhum resultado",
          description: `Nenhuma anomalia encontrada para o veículo com ID "${vehicleIdFilter}".`,
          variant: "info",
        })
      } else {
        toast({
          title: "Detalhes Carregados",
          description: `${filtered.length} anomalias encontradas para o veículo.`,
          variant: "default",
        })
      }
    } else {
      ...
      toast({
        title: "Erro",
        description: "ID do veículo não especificado na URL.",
        variant: "destructive",
      })
    }
  }, [anomalies, vehicleIdFilter])

  // Find the vehicle plate for display in the header
  const vehiclePlate = anomalies.find((a) => a.vehicleId === vehicleIdFilter)?.plate || "N/A"
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground font-jbs">
      ...
      <header className="w-full p-4 border-b bg-white text-jbs-blue shadow-sm">
        <div className="container mx-auto flex flex-col items-center md:flex-row md:justify-between">
          <div className="text-center md:text-left flex items-start gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold">Anomalias do Veículo: {vehiclePlate}</h1>
          ...
          <nav className="flex space-x-4 md:ml-auto items-center">
            ...
            <Link href="/anomaly-report" className="hover:text-jbs-green transition-colors flex items-center gap-1">
              <ArrowLeft className="h-4 w-4" /> Relatório Geral de Anomalias
            </Link>
      ...
      <main className="flex-1 container mx-auto p-4 flex flex-col">
        <h2 className="text-xl font-bold text-center mb-4 text-jbs-blue">
          Detalhes das Anomalias para Placa: {vehiclePlate}
        </h2>
                  ...
                  {filteredAnomalies.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={12} className="h-24 text-center text-muted-foreground">
                        Nenhuma anomalia encontrada para este veículo.
                      </TableCell>
                    </TableRow>
                  ) : (
